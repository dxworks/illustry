        ...
        switch (illustration.type) {
            ...
            case IllustrationTypes.FORCE_DIRECTED_GRAPH: {
                ...
                if (illustration.data && typeof illustration.data === "object" && validate(illustration.data)) {
                    ...
                    throw new TypeError("type force-directed-graph does not correspond to data")
                }
                break
            }
            ...
            case IllustrationTypes.HIERARCHICAL_EDGE_BUNDLING: {
                ...
                if (illustration.data && typeof illustration.data === "object" && validate(illustration.data)) {
                    ...
                    throw new TypeError("type hierarchical-edge-bundling does not correspond to data")
                }
                break
            }
            default: throw new TypeError(`type ${illustration.type} is not one of the known types`)
        }
    } else {
        if (illustration.type && typeof illustration.type === "object" && typeof illustration.type.length === 'number') {
            for (let i = 0; i < illustration.type.length; i++) {
                switch (illustration.type[i]) {
                    ...
                    default: throw new TypeError(`type ${illustration.type[i]} is not one of the known types`)
