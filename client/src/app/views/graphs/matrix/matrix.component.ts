...
export class MatrixComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.data) {
      ...
      this.groups = Array.from(
        new Set(
          this.data.nodes.map((d: any) => {
            return d.group;
          })
        )
      );
      this.selectedGroup1 = Array.from(
        new Set(
          this.data.nodes.map((d: any) => {
            if (d.group === this.groups[0]) return d;
          })
        )
      ).filter((el: any) => {
        return el !== undefined;
      });
      this.selectedGroup2 = Array.from(
        new Set(
          this.data.nodes.map((d: any) => {
            if (d.group === this.groups[1]) return d;
          })
        )
      ).filter((el: any) => {
        return el !== undefined;
  ...
  private createRightHeaderString(spacesForEmptyTd: number, headers: any[]) {
    ...
    headers.forEach((header) => {
      ...
      if (header.style) {
        postHeaderCell =
          postHeaderCell +
          `style="${Object.entries(header.style)
            .map(([k, v]) => `${k}:${v}`)
        ...
        postHeaderCell =
          postHeaderCell +
          `style="font-weight:bold; width: 10%;text-align:center;"` + savedName;
        headerCell = headerCell + postHeaderCell;
      }
    });
  ...
  private createRightPropertiesString(
    ...
    properties?.forEach((prop: any) => {
      ...
      let propCell =
        empty +
        `<td class="sortableCol" style="font-weight: bold; width: 10%; cursor: pointer;text-align:center;">${prop}</td>`;
      finalProduct =
        finalProduct +
        propCell +
        '<td></td>' +
        this.populateRightPropertiesString(group2, prop) +
        '</tr>';
    });
    return `${finalProduct}`;
  }
      ...
      g2.properties?.forEach((g2P: any) => {
        if (g2P.label === property && g2P.style) {
          if (g2P.tooltip) {
            pRow =
              pRow +
              `style=${Object.entries(g2P.style)
                .map(([k, v]) => `${k}:${v}`)
                .join(';')};text-align:center; >${
                g2P.value
              }<span class="tooltiptext">${Object.entries(g2P.tooltip)
                .map(([k, v]) => `${k}:${v}</br>`)
                .join(' ')}</span></td>`;
            ...
            pRow =
              ...
              `style=${Object.entries(g2P.style)
                .map(([k, v]) => `${k}:${v}`)
                .join(';')};text-align:center; >${g2P.value}</td>`;
            finalProduct = finalProduct + pRow;
          }
        } else if (!g2P.style) {
          pRow =
            pRow + `style = "width:10%;text-align:center;">${g2P.value} </td>`;
          finalProduct = finalProduct + pRow;
        }
      });
    });
  ...
  private populateLeftString(
    group1: any[],
    group2: any[],
    links: any,
    propertiesLeftArray: string[]
  ) {
    let finalProduct = '';
    group1.forEach((g1) => {
          ...
          if (g1P.label === p && g1P.style) {
            if (g1P.tooltip) {
              ...
              let col = `<td class="tooltipLeft" style = "${Object.entries(g1P.style)
                ...
                .join(';')} width:10%;text-align:center;" >${
                g1P.value
              }<span class="tooltiptext">${Object.entries(g1P.tooltip)
                .map(([k, v]) => `${k}:${v}</br>`)
                .join(' ')}</span> </td>`;
              ...
              let col = `<td class="tooltipLeft" style = "${Object.entries(
                ...
                .map(([k, v]) => `${k}:${v}`)
                .join(';')} width:10%;text-align:center;" >${g1P.value}</td>`;
              finalProduct = finalProduct + col;
            }
          } else if (g1P.label === p) {
            let col =
              firstCol +
              `<td style = "width:10%;text-align:center;">${g1P.value} </td>`;
            finalProduct = finalProduct + col;
          }
        });
      });
      group2.forEach((g2) => {
        finalProduct =
          finalProduct + this.populateRightString(g1.id, g2.id, links);
      });
  ...
  private populateRightString(
    group1Name: string,
    group2Name: string,
    links: any
  ) {
    let finalProduct = '';
    const link = Array.from(new Set(links))
      .map((d: any) => {
        if (d.source === group1Name && d.target === group2Name) return d;
      })
      .filter((el: any) => {
        return el !== undefined;
      })[0];
    if (link !== undefined && link !== null && this.clicked === false) {
      ...
      if (!link.style) {
        ...
        if (link.tooltip) {
          rRow =
            rRow +
            `style= ${Object.entries(link.style)
              .map(([k, v]) => `${k}:${v}`)
              .join(';')};width:10%;text-align:center;>${
              link.value
            }<span class="tooltiptext">${Object.entries(link.tooltip)
              .map(([k, v]) => `${k}:${v}</br>`)
              .join(' ')}</span></td>`;
          finalProduct = finalProduct + rRow;
        } else {
          rRow =
            ...
            `style= ${Object.entries(link.style)
              .map(([k, v]) => `${k}:${v}`)
              .join(';')};width:10%;text-align:center;>${link.value}</td>`;
          finalProduct = finalProduct + rRow;
        }
      }
      ...
      rRow =
        ...
        `style= ${Object.entries(link.style)
          .map(([k, v]) => `${k}:${v}`)
          .join(';')};width:10%;text-align:center;></td>`;
      finalProduct = finalProduct + rRow;
      if (!link.style) {
        rRow = rRow + '>' + '</td>';
        finalProduct = finalProduct + rRow;
  ...
  private sortTable(n: any) {
    ...
    while (switching) {
      ...
      for (i = rows.length - sortusRows.length; i < rows.length - 1; i++) {
  ...
  private createHeadersAndPropertiesString(
    group1: any[],
    group2: any[],
    links: any[]
  ) {
    ...
    const rightHeader: string = this.createRightHeaderString(
      propertiesLeftArray.length,
      group2
    );
    const rightProperties: string = this.createRightPropertiesString(
      propertiesLeftArray.length,
      propertiesRightArray,
      group2
    );
    const leftHeader = this.createLeftHeaderString(propertiesLeftArray);
    const populate = this.populateLeftString(
      group1,
      group2,
      links,
      propertiesLeftArray
    ...
    const tableString: string =
      rightHeader + rightProperties + leftHeader + populate;
    return tableString;
  }
...
#magicbutton {
  ...
  margin-right: auto;
    ...
    this.tableString =
      `${style}<table id ="myTable" style= "border-spacing: 0;width: 100%;border: 1px solid #ddd";>` +
      this.createHeadersAndPropertiesString(
        this.selectedGroup1,
        this.selectedGroup2,
        this.links
      ); //+ `</table>` + `<div class="buttonCenter">${button}</div>`;
