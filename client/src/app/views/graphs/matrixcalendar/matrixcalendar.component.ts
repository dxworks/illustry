...
export class MatrixcalendarComponent {
   ...
   */
  ngOnChanges() {
    if (!this.data.calendarData) { return; }
  ...
  onResize(event: any) {
    this.calculateDimensions();
    if (!!this.data.calendarData && !!this.data.calendarData[0] && !!this.data.calendarData[0].summary) {
      this.drawChart();
    }
  };
  ...
  updateDataSummary() {
    ...
    if (this.data.calendarData[0] && !this.data.calendarData[0].summary) {
      this.data.calendarData.map((d) => {
   ...
   */
  drawChart() {
    if (!this.svg || !this.data.calendarData || !this.selected) { return; }
    ...
    switch (this.overview) {
      case OverviewType.global:
        ...
        this.onChange.emit({
          ...
          start: this.data.calendarData[0].date,
          end: this.data.calendarData[this.data.calendarData.length - 1].date,
        })
        break;
  ...
  drawGlobalOverview() {
    ...
    var start: any = moment(this.data.calendarData[0].date).startOf('year');
    var end: any = moment(this.data.calendarData[this.data.calendarData.length - 1].date).endOf('year');

    // Define array of years and total values
    var data = this.data.calendarData;
  ...
  drawYearOverview() {
    ...
    var year_data = this.data.calendarData.filter(d => {
    ...
    this.labels.selectAll('.label-month')
      ...
      .on('click', (d: any) => {
        ...
        // Check month data
        var month_data = this.data.calendarData.filter((e: any) => {
  ...
  drawMonthOverview() {
    ...
    var month_data = this.data.calendarData.filter(d => {
    ...
    item_block.selectAll('.item-block-rect')
      ...
      .attr('x', function (d: CalendarHeatmapDataSummary) {
      ...
      .attr('width', function (d: CalendarHeatmapDataSummary) {
        var total = parseInt(d3.select(this.parentNode).attr('total'));
        itemScale.domain([0, total]);
    ...
    this.labels.selectAll('.label-week')
      ...
      .on('click', (d: any) => {
        ...
        // Check week data
        var week_data = this.data.calendarData.filter((e: any) => {
  ...
  drawWeekOverview() {
    ...
    var week_data = this.data.calendarData.filter(d => {
  ...
  drawDayOverview() {
    ...
    if (!Object.keys(this.selected).length) {
      this.selected = this.data.calendarData[this.data.calendarData.length - 1];
    }
